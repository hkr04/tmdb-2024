...
public class SelectImpl implements edu.whu.tmdb.query.operations.Select {
    ...
    private SelectResult limit(int limit, SelectResult selectResult) throws TMDBException {
        if (limit < 0) {
            throw new TMDBException(ErrorList.LIMIT_VALUES_ERROR, "No qualified limit: " + limit);
        }
        TupleList limit_tuple = selectResult.getTpl();
        List<Tuple> limit_tuplelist = limit_tuple.tuplelist;
        // 使用 Java 流将元组的数量限制到指定的 limit
        List<Tuple> new_trl = limit_tuplelist.stream().limit(limit).collect(Collectors.toList());
        limit_tuple.tuplelist = new_trl;
        int resize = Math.min(new_trl.size(), limit);
        limit_tuple.tuplenum = resize;
        return selectResult;
    }

    private SelectResult groupByElicit(PlainSelect plainSelect, HashMap resultMap, SelectResult selectResult)
    ...
    private ArrayList<Object> solveAggregationFunction(HashMap<Object, ArrayList<Tuple>> resultMap, String funcName,
                                                       int pIndex) {
    ...
    public SelectResult projection(PlainSelect plainSelect, SelectResult entireResult) throws TMDBException {
        ArrayList<SelectItem> selectItemList = (ArrayList<SelectItem>) plainSelect.getSelectItems(); // 获取from后的查询项
        // (形如from id,
        // value as v)
    ...
    private void projectSelectExpression(SelectItem item, SelectResult entireResult,
        ...
        // 3.resTupleList赋值
        //含有两个元素的列表，结构为[tableName, columnName]
        ArrayList<String> TableColumn = new ArrayList<>();
        // 调用attributeParser();表达式解析函数，给定表达式（形如value1 + value2 as sum）* 将其第一个属性的表名和属性名赋值给相应变量
        attributeParser(attrName, TableColumn);

        //Formula 执行表达式,对表达式进行解析，获取该列的值
        ArrayList<Object> thisColumn=(new Formula()).formulaExecute(selectItem.getExpression(),entireResult);
        // 调用getIndexInEntireResult();
        // 给定查询属性的属性名和表名，返回属性在完全的查询结果中的位置,找到表达式对应属性在原元组对应的下标.
        ...
        int length = resTupleList.tuplelist.size();
        // 5. 将结果插入 resTupleList
        for (int i = 0; i < length ; i++) {
            Tuple tuple = resTupleList.tuplelist.get(i);
            Tuple entire_tuple = entireResult.getTpl().tuplelist.get(i);
            tuple.classId = entire_tuple.classId;
            tuple.tupleId = entire_tuple.tupleId;
            tuple.tuple[indexInResult] = thisColumn.get(i);
            tuple.tupleIds[indexInResult] = entire_tuple.tupleIds[IndexInEntireResult];
        }
    }
    ...
    private int projectAllTableColumns(SelectItem item, SelectResult entireResult, SelectResult projectResult,
        ...
        for (int i = 0; i < entireResult.getClassName().length; i++) {
            ...
            if (className.equals(table.getName())
                ...
                for (int j = 0; j < resTupleList.tuplelist.size(); j++) {
                    resTupleList.tuplelist.get(j).tuple[indexInResult] = entireResult.getTpl().tuplelist
                            .get(j).tuple[i];
                    resTupleList.tuplelist.get(j).tupleIds[indexInResult] = entireResult.getTpl().tuplelist
                            .get(j).tupleIds[i];
                }
                indexInResult++;
            }
    ...
    public SelectResult from(PlainSelect plainSelect) throws TMDBException {
        ...
        ArrayList<ClassTableItem> classTableItemList = memConnect.copyClassTableList(fromItem); // 获取class
        // item信息，对应于select输出列表的表头
    ...
    public SelectResult Operate(SelectResult selectResult1, SelectResult selectResult2, SetOperation setOperation)
            throws TMDBException {
        if (selectResult1.getAttrname().length != selectResult2.getAttrname().length)
            throw new TMDBException();
        // 根据setOperation的种类进行操作划分
        switch (setOperation.toString()) {
            case "UNION":
                return union(selectResult1, selectResult2);
            case "INTERSECT":
                return intersect(selectResult1, selectResult2);
            case "EXCEPT":
                return except(selectResult1, selectResult2);
            case "MINUS":
                return minus(selectResult1, selectResult2);
        }
        return null;
    }
    ...
    public SelectResult values(ValuesStatement valuesStatement) {
        ...
        for (int i = 0; i < tupleList.tuplelist.get(0).tuple.length; i++) {
            ClassTableItem classTableItem = new ClassTableItem("", -1, tupleList.tuplelist.get(0).tuple.length, i,
                    "attr" + i, tupleList.tuplelist.get(0).tuple.getClass().getSimpleName(), "", "");
            ...
            for (Tuple tuple : rightTupleList.tuplelist) {
                for (Tuple t : leftTupleList.tuplelist) {
                    Tuple newTuple = new Tuple(
                            Stream.concat(Arrays.stream(t.tuple), Arrays.stream(tuple.tuple)).toArray());
                    newTuple.tupleIds = IntStream.concat(Arrays.stream(t.tupleIds), Arrays.stream(tuple.tupleIds))
                            .toArray();
        ...
        for (ClassTableItem item : MemConnect.getClassTableList()) {
            if (item.classname.equals(fromItem.toString())) {
                ...
                for (Column column : columnList) {
                    ...
                    if (c.getTable() != null && !(c.getTable().equals(fromItem.toString())
                            && c.getTable().equals(fromItem.getAlias().getName())))
                        continue;
